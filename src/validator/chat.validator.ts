...
import { NewChat, CHAT_TYPE, NewChatMessages } from "../model/chat"

// a single message validation
const chatMessageBody: Record<keyof NewChatMessages, any> = {
    message: joi.string().trim(),
    senderOrgId: joi.string().custom(objectId).trim(),
}

// chat validation
const chatBody: Record<keyof NewChat, any> = {
    buyerOrgId: joi.string().custom(objectId).trim(),
...
export const createChat = {
    body: joi.object().keys({
        ...
        messages: joi.array().items(
            joi.object().keys(otherChatMessageBody).options({ presence: 'required' })
        ).required(),
...
export const updateChat = {
    ...
    body: joi.object().keys(otherChatBody).min(1),
}
